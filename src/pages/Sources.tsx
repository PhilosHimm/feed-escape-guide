...
const Sources = () => (
  <div className="py-12 px-4 max-w-3xl mx-auto">
    ...
    <ul className="mt-4 space-y-5">
      {sources.map((src, idx) => (
        <li key={idx}>
          <div className="mb-2 text-lg text-muted-foreground">{src.desc}</div>
          <ul className="pl-3 list-disc space-y-1">
            {src.links.map((link, i) => (
              <li key={i}>
                <a href={link.url} className="underline text-purpleblue-700" target="_blank" rel="noopener noreferrer">{link.label}</a>
              </li>
            ))}
          </ul>
        </li>
